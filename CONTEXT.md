...
| V0.29 | 魔药失控模型：放弃硬门槛，风险在服食时（已完成） |
| V0.30 | 原著失控模型：诱因×三档后果 + 精神死亡终局（已完成） |
| V0.30-W | Web 端序列同步：途径选择+晋升+失控三档落地（已完成） |
| V0.31 | 廷根岁时节令：4 节日（万象/长夜/丰收/岁末）+ 日历条件（已完成） |
| 后续 | 双端数据单一化、更多 NPC/事件内容 |
...
22. **V0.20 城市见闻有时效**：失踪新闻只在刚发生 3 天内是头条，之后淡出（城市继续运转）——避免"同一句复读一年"的机器感；经济/组织/季节轮换发声，存档保留公告历史。
...
31. **V0.28 复查修复（重要）**：① **Python 事件系统此前没有 once_tag 防重**（Web 有）——途径/晋升/分支事件会无限次触发；现补 `EventNode.once_tag` + available_nodes 过滤 + apply/apply_choice 触发即打标，与 Web 语义一致（错过即错过）。② `_sequence` 只允许晋升（target<current）防降级/重设。教训：内容迭代后必须回归"一次性事件"机制，两端语义要保持一致。
32. **V0.29 魔药失控模型（用户修正核心语义）**：原著风险是**失控**而非"够不够格"——灵性不足**事件照常出现**（不再硬门槛，V0.28 的 min_spirituality 已移除），风险全在服食时：`drink_potion()` 成分 = 基线（精神污染/相性差）+ 灵性达标安全加成（−35%）−/＋灵性外溢惩罚（每差一格比例 +，封顶 70%）。验收：灵性3 失控率 92%（十死一生）、灵性60 失控率 2%（永远有极小风险，符合原著）；失控标签「魔药反噬/灵性外溢/精神污染」+ 疯狂+20 + 晋升失败，平稳则晋升+序列加成。`_potion` 特效键（替代静态 `_sequence`+固定 effects）；can_consume 降级为提示（不再阻止）。
33. **V0.30 原著失控模型（用户给全设定，诱因×后果）**：诱因逐项累积（`_additive_risk`）：①魔药本身（相性差基线 6% + 高序列污染 +6%）②自身状态（灵性枯竭每差门槛比例 +45%×3 封顶 0.7、压力>60 +12%、mood<25 +10%、疯狂>60 +15%、濒危≥80 +35%）③未消化强升（距上次晋升<30 天 +20%）④外界污染（带高位格类标签 +8%）⑤跨途径（+60%）——总风险封顶 **0.97 永不 100%**。后果三档 roll（对应原著三种表现，权重 55:32:13）：**B 当场发狂**（疯狂+14、压力+12、体力-6）/**A 人格被扭曲**（疯狂+16、魅力-6，冷酷标签）/**S 精神死亡·身体崩溃**（疯狂+30、健康-50、体力-20，**角色 dead + death_reason**，引擎 process_action 拦截死后再行动）。教训：后果数值需**叠加**基疯狂而非覆盖（曾 update 覆盖致 B 只有 14）。
34. **V0.30-W Web 序列同步**：Web 端此前 pathway **从未被设置**（途径选择事件缺失→占卜行动/途径加成永久沉睡）。补：`pathway_choice` 事件（真相抉择"加入组织"后解锁，`_pathway` 特效键设 pathway+sequence=9）+ 3 个晋升事件（`_potion` → `drinkPotionWeb()`，镜像 Python 诱因/三档/死亡语义，只做 9→8 一阶够玩）+ renderPathway 显示"小丑·序列8"；死亡走 alert 终局提示。验证：200 次低灵性喝药得 晋升8/发狂102/扭曲64/死亡26。
35. **V0.31 节日 = calendar 条件**（特定月日，每年可再遇 cooldown=360，区别于季节池）：`conditions_met` + normalize + schema 都加 `calendar{month,day}`；Python `festival_days` 图 4 节点（万象3/1、长夜7/30、丰收9/21、岁末12/25），Web 同步 4 事件（canEventTriggerAt 加 calendar 分支，trustEffects 用 Web 联系人 key neighbor/newsboy——Web 与 Python 联系人 key 不同，是双端数据不同步的又一例证）。教训：Web contacts key（neighbor）≠ Python npcs id（sha_ren_neighbor），任何 trustEffect 都要各自对表。
...
```
V0.14 World Tick 2.0        ✅ 单一入口编排（时间→NPC→地点→经济→关系→事件）
V0.15 NPC Life 1.0（6 小版） ✅ 状态/需求/日程2.0/行为评分/经济联动/互动/事件钩子
V0.16 世界事件驱动事件图     ✅ NPC_MISSING → 失踪链激活
V0.20 城市感知层            ✅ 每日见闻（时效、轮换、存档）
V0.21 非凡途径（克制版）     ✅ 占卜家/观众/不眠者
V0.22 数据校验工具链        ✅ JSON Schema + 语义校验（tools/check_data.py）
```

### 未来的迭代方向（候选，按"给玩家长线价值"排序）

1. **一年人生总结结局**（低投入高回报）：仿照"生命周期终点回望"，玩家过完一年给一份叙事式总结——关键事件、结识的人、途径之路、疯狂弧线。当前 `update_world` 已跟踪大部分数据，用已有的 journal/tags/clues 生成"年度回顾"报告即可。**可立刻做，给玩家重玩动力。**
2. **途径深化（低序列能力）**：占卜家→"占卜"行动（消耗灵性/理性换预知线索）、观众→"读心"选择（社交加信任看破谎言）、不眠者→"夜间行动"特权（夜晚可调查且不疲劳）。每个途径 2-3 个专属互动，接到现有 behavior/事件系统。
3. **NPC 长期记忆（轻量）**：V0.15 有意跳过"长期记忆"（规格说留给以后）——现在可以加：NPC 记住"玩家帮过/坑过 Ta"（影响 trust 幅度），"昨天在家乡害死的人"这类事件回声。规则驱动、不 AI（记忆=事件→关系 delta 的加权表）。
4. **事件图分支深化（V0.16 的"图级分支"）**：图内选择走不同子图（现在节点内 choice 只改属性/推进到下节点，还没做"选择后整条后续链不同"）。
5. **沉淀玩法：每日多事件**（一天可遭遇 2-3 个独立事件而非 1 个）。

### 不做/慎做（始终贯彻的边界）

- ❌ AI/NLP 直接改世界规则（用户铁律：数据库决定真相、引擎决定能不能发生、叙事层负责怎么讲）
- ❌ 完整 22 途径树 / 全城经济 / 真实家庭树 / 数百 NPC（V0.15 规格明确砍掉）
- ⚠️ 线上部署已作废，无此待办
- ⚠️ 数据双写（Web app.js 内嵌 vs Python data/）仍是隐患，重构前先做"单一数据源"（如 Web 从 data/*.json 读取）

---

## 八、待办/边界/坑

- ✅ **线上部署已作废**：原 `api.yapasky.dpdns.org/life-simulator/` 不再维护，README 已标注"本地游玩"（`web/index.html` 网页版 / `python run.py` CLI 版）。不再有"同步上线"待办。
